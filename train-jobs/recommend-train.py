...
def train(args: argparse.Namespace, profiler: MLProfiler = None) -> None:
    ...
    def load_checkpoint_state(ckpt):
        scheduler_state = ckpt.get("lr_scheduler_state_dict")
        if scheduler_state is not None:
            lr_scheduler.load_state_dict(scheduler_state)
        checkpoint_runtime["steps"] = ckpt.get("steps", 0)
    ...
    manager = CheckpointManager(
        ...
        initial_epoch=1,
        save_state_hook=save_checkpoint_state,
        load_state_hook=load_checkpoint_state,
    )

    if checkpoint_resume_path and os.path.exists(checkpoint_resume_path):
        logger.log(f"resuming training from {checkpoint_resume_path}")
    ...
    progress_bar = tqdm(total=num_training_steps, initial=checkpoint_runtime["steps"])
    ...
    for epoch in range(start_epoch, run_epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if epoch == start_epoch and batch_idx == 0:
                logger.log(f"{FIRST_BATCH_LOG_MARKER} epoch={epoch} batch={batch_idx}")
